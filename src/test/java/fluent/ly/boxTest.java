...
@SuppressWarnings("static-method") public class boxTest {
  @Test public void boxBoolTest() {
    assert Box.it(true).booleanValue();
    assert !Box.it(false).booleanValue();
  }

  @Test public void boxBoolArrayTest() {
    final Boolean[] boxa = Box.it(new boolean[] { true, false });
    assert (boxa[0].booleanValue());
    assert !(boxa[1].booleanValue());
  }
  ...
  @Test public void itBoolTest() {
    assert Box.it(Box.it(true).booleanValue()).booleanValue();
    assert !Box.it(false).booleanValue();
  }

  @Test public void itBoolArrayTest() {
    final Boolean[] ita = Box.it(new boolean[] { true, false });
    for (int ¢ = 0; ¢ < 2; ++¢)
      if (¢ == 0)
        assert ita[0].booleanValue();
      else if (¢ == 1)
        assert !ita[1].booleanValue();
  }
