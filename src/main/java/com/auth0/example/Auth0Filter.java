...
@WebFilter(urlPatterns = "/logged/*") public class Auth0Filter implements Filter {
   ...
   * response, final FilterChain next) throws IOException, ServletException { if
   * ((String) SessionUtils.get((HttpServletRequest) r, "accessToken") != null ||
   * (String) SessionUtils.get((HttpServletRequest) r, "idToken") != null) {
   * next.doFilter(r, response);} else ((HttpServletResponse)
